    ...
    public partial class MainForm : Form
    {
        ...
        private void editOperation_OperationMade(EditOperationControl sender, ChartOperation e)
        {
            ...
            if (chart == null)
            {
                try
                {
                    StreamReader reader = new StreamReader(fileInTextBox.Text);
                    XmlSerializer serialiser = new XmlSerializer(typeof(CMap));
                    chart = (CMap)serialiser.Deserialize(reader);
                    reader.Close();
                }
                ...
                {
                    if (ex is FileNotFoundException || ex is DirectoryNotFoundException || ex is ArgumentException || ex is IOException)
                    {
